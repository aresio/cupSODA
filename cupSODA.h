/*
cupSODA.h: porting of LSODA to CUDA. Implementation of device-side
...
__constant__ unsigned int DEV_CONST_ODELUN = 0;
__constant__ unsigned int DEV_CONST_JACLUN = 0;
...
void SetConstants( unsigned int species, unsigned int reactions, unsigned int ode_lun, unsigned int jac_lun, unsigned int cs_lun, unsigned int time_ins, unsigned int reps, unsigned int exps, unsigned int act_threads, bool);
...
struct myJex
{
	__device__ void operator()(int *neq, double *t, double *y, int ml, int mu, double *pd, int nrowpd, param_t* JAC, unsigned int* JAC_offset, double* costanti/*, void *otherData*/)
	{
...
__device__ int dlsoda_(myFex, int *, double *, double *, double *, int *, double *, double *, int *, int *, int *, double *, int *, int *, int *, myJex, int *, struct cuLsodaCommonBlock *, int* debug, char* comp_ode,  param_t* flattenODE, unsigned int* offsetODE,  double* costanti);
...
__host__ __device__ void cuLsodaCommonBlockInit(struct cuLsodaCommonBlock *common, unsigned int);


/*
	This is the entrypoint of cuLSODO.
	All these state variables must be changed to multidimensional arrays, in order to exploit the SIMD architecture.
*/
